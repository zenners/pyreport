...
def get_data1():
    ...
    # pandas to excel
    writer = pd.ExcelWriter(output, engine='xlsxwriter')
    # headers = ['paymentSource', 'paymentSource', 'postedDate', 'orNo', 'orNo', 'loanAccountNo',
    #            'customerName', "amountApplied", 'amountApplied', 'amountApplied', "mobileNo", "mobileNo", "mobileNo"]
    df = pd.DataFrame(sortData)
    ...
    # return jsonify(greater_than_zero)
        ...
        df['penalty'] = ''
        sum = pd.Series(df['amountApplied']).sum()
        df = df[['paymentSource', 'cci', 'postedDate', 'orNo', 'checkNo', 'loanAccountNo', 'customerName',
                 'total', 'amountApplied', 'check', 'principal', 'interest', 'advance', 'penalty']]
    df.to_excel(writer, startrow=5, merge_cells=False, index=False, sheet_name="Sheet_1")
    ...
    worksheet = writer.sheets["Sheet_1"]
    worksheet.merge_range('A1:N1', 'RADIOWEALTH FINANCE COMPANY, INC.', merge_format3)
    worksheet.merge_range('A2:N2', 'RFC360 Kwikredit', merge_format1)
    worksheet.merge_range('A3:N3', 'Daily Cash Collection Report', merge_format3)
    worksheet.merge_range('A4:N4', xldate_header, merge_format1)

    worksheet.merge_range('A6:A7', 'Payment Type', merge_format5)
    worksheet.merge_range('B6:B7', 'CCI', merge_format5)
    worksheet.merge_range('C6:C7', 'Date', merge_format5)
    worksheet.merge_range('D6:D7', 'OR #', merge_format5)
    worksheet.merge_range('E6:E7', 'Check #', merge_format5)
    worksheet.merge_range('F6:F7', 'Loan Account Number', merge_format5)
    worksheet.merge_range('G6:G7', 'Customer Name', merge_format5)
    worksheet.merge_range('H6:J6', 'AMOUNT', merge_format5)
    worksheet.write('H7', 'Total', merge_format5)
    worksheet.write('I7', 'Cash', merge_format5)
    worksheet.write('J7', 'Check', merge_format5)
    worksheet.merge_range('K6:N6', 'LOAN REPAYMENT', merge_format5)
    worksheet.write('K7', 'Principal', merge_format5)
    worksheet.write('L7', 'Interest', merge_format5)
    worksheet.write('M7', 'Advances', merge_format5)
    worksheet.write('N7', 'Penalty (5%)', merge_format5)

    worksheet.merge_range('A{}:N{}'.format(count - 1, count - 1), nodisplay, merge_format1)
    worksheet.write('G{}'.format(count + 1, count + 1), 'TOTAL NET CASH', merge_format3)
    worksheet.write('H{}'.format(count + 1), sum, merge_format4)
    worksheet.write('I{}'.format(count + 1), sum, merge_format4)
    worksheet.write('J{}'.format(count + 1), sum, merge_format4)
    worksheet.merge_range('A{}:N{}'.format(count + 3, count + 3), 'Report Generated By :', merge_format2)
    worksheet.merge_range('A{}:N{}'.format(count + 4, count + 5), name, merge_format2)
    worksheet.merge_range('A{}:N{}'.format(count + 7, count + 7), 'Date & Time Report Generation ({})'.format(dateNow),
                          merge_format2)
    # the writer has done its job
    writer.close()
