...
def collectionreport():
    ...
    headers = ["App Id", "Mobile Number", "Loan Account No", "Customer Name", "FDD", "DD", "PNV", "MLV", "MI", "Term",
    ...
    worksheet.merge_range('E{}:F{}'.format(count + 1, count + 1), 'TOTALS', merge_format3)
    worksheet.write('G{}'.format(count + 1), pnvsum, merge_format4)
    worksheet.write('H{}'.format(count + 1), mlvsum, merge_format4)
    worksheet.write('I{}'.format(count + 1), misum, merge_format4)
    worksheet.write('K{}'.format(count + 1), sumOfPenaltysum, merge_format4)
    worksheet.write('L{}'.format(count + 1), amountDuesum, merge_format4)
    worksheet.write('O{}'.format(count + 1), outstandingBalancesum, merge_format4)
    worksheet.write('Q{}'.format(count + 1), totalPaymentsum, merge_format4)
...
def accountingAgingReport():
    ...
    worksheet.merge_range('M{}:N{}'.format(count + 1, count + 1), 'TOTALS', merge_format3)
...
def operationAgingReport():
    ...
    worksheet.merge_range('W{}:X{}'.format(count + 1, count + 1), 'TOTALS', merge_format3)
...
def get_data1():
    ...
    # return jsonify(greater_than_zero)
        ...
        df['paymentCheck'] = df['paymentCheck'].astype(float)
        df['paidPrincipal'] = df['paidPrincipal'].astype(float)
        df['paidInterest'] = df['paidInterest'].astype(float)
        df['paidPenalty'] = df['paidPenalty'].astype(float)
        conditions = [(df['paymentSource'] == 'Check')]
        df['pipTotal'] = df['paidPrincipal'] + df['paidInterest'] + df['paidPenalty']
        df['total'] = np.select(conditions, [df['paymentCheck']], default=df['amount'])
        advancesconditions = [(df['pipTotal'] == 0)]
        df['advance'] = np.select(advancesconditions, [df['total']], default=df['total'] - df['pipTotal'])
        advancesconditions1 = [(df['advance'] < 0)]
        df['advance'] = np.select(advancesconditions1, [0], default=df['advance'])
        amountsum = pd.Series(df['total']).sum()
        cashsum = pd.Series(df['amount']).sum()
        checksum = pd.Series(df['paymentCheck']).sum()
        advancessum = pd.Series(df['advance']).sum()
        ...
        df = df[['paymentSource', 'cci', 'orDate', 'orNo', 'checkDate', 'checkNo', 'loanAccountNo', 'customerName',
                 'total', 'amount', 'paymentCheck', 'paidPrincipal', 'paidInterest', 'advance', 'paidPenalty']]
    ...
    worksheet.write('N7', 'Advances', merge_format5)
    worksheet.write('O7', 'Penalty (5%)', merge_format5)

    worksheet.merge_range('A{}:O{}'.format(count - 1, count - 1), nodisplay, merge_format1)
    worksheet.write('H{}'.format(count + 1, count + 1), 'TOTALS', merge_format3)
    ...
    # worksheet.write('K{}'.format(count + 1), sum, merge_format4)
...
def get_monthly():
    ...
    worksheet.write('C{}'.format(count + 1), 'TOTALS', merge_format3)
...
def get_booking():
    ...
    worksheet.write('D{}'.format(count + 1), 'TOTALS', merge_format3)
...
def get_incentive():
    ...
    worksheet.merge_range('G{}:H{}'.format(count + 1, count + 1), 'TOTALS', merge_format3)
...
def get_mature():
    ...
    worksheet.merge_range('D{}:E{}'.format(count + 1, count + 1), 'TOTALS', merge_format3)
