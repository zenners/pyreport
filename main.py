...
def get_data1():
    ...
    # return jsonify(greater_than_zero)
        ...
        df['diff'] = df['total'] - df['pipTotal']
        advancesconditions = [(df['pipTotal'] == 0)]
        df['advances'] = np.select(advancesconditions, [df['total']], default=df['diff'])
        # advancesconditions1 = [(df['advances'] < 0)]
        # df['advances'] = np.select(advancesconditions1, [0], default=df['advance'])
    ...
    worksheet.write('P7', 'Payment Amount', merge_format5)
    worksheet.write('Q7', 'PIP Total', merge_format5)
    worksheet.write('R7', 'Advances', merge_format5)
